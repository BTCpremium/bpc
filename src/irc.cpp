...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("BitcoinPremium-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BitcoinPremiumTEST2\r");
            Send(hSocket, "WHO #BitcoinPremiumTEST2\r");
        } else {
            // randomly join #BitcoinPremium00-#BitcoinPremium05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BitcoinPremium%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BitcoinPremium%02d\r", channel_number).c_str());
        }
