BitcoinPremium-qt: Qt4 GUI for BitcoinPremium
===============================
...
Alternatively, install Qt Creator and open the `BitcoinPremium-qt.pro` file.

An executable named `BitcoinPremium-qt` will be built.
...
A warning for people using the *static binary* version of BitcoinPremium on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of BitcoinPremium is linked against libdb5.3.
...
installed causes BitcoinPremium-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
